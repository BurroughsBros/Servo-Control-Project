...
* PD Controller Implementation for STM32

*

* This program implements a discrete-time PD controller on an STM32

* microcontroller. The controller receives a desired position from the
  user via

* UART, reads the current motor position using an encoder, computes the
  error, and

* applies a control law to adjust the PWM signal sent to the motor.

*/

// Global Variables

int x = 0; *// Stores encoder count*

int test = 0; *// Debugging variable*

// Control and ADC variables

**volatile** int pwmValue = 0; *// PWM duty cycle output*

**volatile** int refCount = 0; *// Reference (desired) position input
from user*

float converted = 0; *// Scaled encoder count*

float ek = 0; *// Current error*

float mk = 0; *// Control output (before saturation)*

float ek1 = 0; *// Previous error (for PD calculation)*

int plantPWM = 0; *// Final PWM command sent to motor*

float adValue = 0.0; *// ADC value placeholder (not currently used)*

// PD Controller Gains (Adjust as needed)

**const** float kp = 9.396; *// Proportional gain*

**const** float kd = 0.733; *// Derivative gain*

**const** float Ts = 0.0720; *// Sampling period (seconds)*

// Computed Coefficients for Discrete-Time Controller
...
*/\* Initialize motor control and communication */*

motor_initEncoder(htim2, TIM_CHANNEL_1, TIM_CHANNEL_2); *// Initialize
quadrature encoder*

motor_initPWM(htim1, TIM_CHANNEL_1, TIM_CHANNEL_2); *// Initialize PWM
for motor control*

terminal_init(huart2); *// Initialize UART terminal*

HAL_TIM_Base_Start_IT(&htim3); *// Start timer interrupt for control
loop*
...
*/\* Main loop - receive user input and update reference position */*
...
terminal_receive(rxBuff, **sizeof**(rxBuff)); *// Get input from user*

sscanf(rxBuff, "%d", &refCount); *// Convert string input to integer*

sprintf(txBuff, "**\r\n**Value Entered: %d**\r\n**", refCount);

terminal_print(txBuff); *// Display entered value*

HAL_Delay(2500); *// Wait before next input cycle*
...
/* Timer Interrupt - Runs every Ts seconds */

void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {

#ifdef CONTROLLER *// Ensure the controller runs only if CONTROLLER is
defined*

*/\* Read encoder count (current motor position) */*

x = motor_getCount();

*/\* Convert encoder count to physical units (degrees, etc.) */*

converted = (motor_getCount()) * 0.036; *// Example scaling factor*

*/\* Compute error signal (difference between desired and actual
position) */*

ek = (float)(refCount - converted);

*/\* Compute PD control output using discrete-time equation */*

mk = alpha1 * ek - alpha2 * ek1;

*/\* Apply saturation to limit PWM output */*
...
*/\* Apply control output to motor (handle direction separately) */*

**if** (plantPWM >= 0) {

motor_PWMSetForward(plantPWM); *// Move forward*

} **else** {

motor_PWMSetBackward(abs(plantPWM)); *// Move backward*
